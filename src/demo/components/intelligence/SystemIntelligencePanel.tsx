...
import type { IntelTabId } from '../../types';
import { Hash, Brain, Globe, TrendingDown, Flame, Beaker } from 'lucide-react';

const TABS: { id: IntelTabId; label: string; icon: typeof Hash }[] = [
...
export function SystemIntelligencePanel() {
  const { intelTab, setIntelTab } = useDemo();
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="flex items-center gap-0.5 px-2 pt-2 pb-1 border-b border-border/20 flex-shrink-0 overflow-x-auto">
        {TABS.map(tab => {
          const Icon = tab.icon;
          const isActive = intelTab === tab.id;
          return (
            <button
              key={tab.id}
      ...
      <ScrollArea className="flex-1 px-3 py-3">
        <AnimatePresence mode="wait">
          ...
          >
            {intelTab === 'ledger' && <DecisionLedgerPanel />}
            {intelTab === 'meta' && <MetaReliabilityPanel />}
            {intelTab === 'regime' && <RegimeGeometryPanel />}
            {intelTab === 'decay' && <ConfidenceDecayPanel />}
            {intelTab === 'drift' && <AdversarialDriftPanel />}
            {intelTab === 'experiments' && <ExperimentPanel />}
          </motion.div>
        </AnimatePresence>
      </ScrollArea>
    </div>
